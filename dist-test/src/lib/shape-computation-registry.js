 ...
 */
export const shapeComputationRegistry = {
    // Input layer computation
    input_layer: (inputShapes, params) => {
    ...
    flatten_layer: (inputShapes, params) => {
        if (inputShapes.length !== 1)
            return null;
        const inputShape = inputShapes[0];
    ...
    global_avg_pool_layer: (inputShapes, params) => {
        if (inputShapes.length !== 1)
            return null;
        const inputShape = inputShapes[0];
        if (inputShape.length === 3) {
    ...
    preserve_shape: (inputShapes, params) => {
        if (inputShapes.length !== 1)
            return null;
 ...
 */
function parseUpSamplingSize(sizeStr) {
    try {
 ...
 */
export function getShapeComputationFunction(shapeComputationName) {
    return shapeComputationRegistry[shapeComputationName] || null;
}
/**
 * Compute layer output shape using YAML-driven shape computation
 */
export function computeLayerShape(layerType, inputShapes, params, shapeComputationName) {
 ...
 */
function getDefaultShapeComputation(layerType) {
    const defaults = {
        'Input': 'input_layer',
        'Dense': 'dense_layer',
        'Output': 'dense_layer',
