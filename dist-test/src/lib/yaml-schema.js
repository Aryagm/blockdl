...
export function migrateLegacyConfig(legacyConfig) {
    ...
    if (config.layers) {
        ...
        for (const [key, layer] of Object.entries(layers)) {
            const layerObj = layer;
            const metadata = layerObj.metadata;
            const keras = layerObj.keras;
            enhanced.layers[key] = {
                metadata: {
                    ...metadata,
                    info: '/docs/general-info.md' // Default layer info - only for layers
                },
                visual: layerObj.visual,
                parameters: layerObj.parameters,
                frameworks: {
                    // Convert legacy keras section to new frameworks structure
                    keras: {
                        import: keras?.import || '',
                        template: keras?.code_template || '',
                        shape_computation: keras?.shape_computation || ''
                    }
                }
            };
        }
