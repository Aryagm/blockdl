 ...
 * 
 * Provides clean utilities for generating parameter display strings
 * and counting parameters for layer visualization in the UI.
 */
...
const computeInputShape = (params: Record<string, LayerParamValue>): string => {
...
export function getParameterDisplayValues(layerType: string, params: Record<string, LayerParamValue>): string[] {
  ...
  if (layerType === 'Input') {
    const inputType = String(params.inputType || 'image_grayscale')
    return [
      `shape: ${computeInputShape(params)}`,
