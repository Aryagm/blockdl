...
/**
 * Convert legacy YAML format to enhanced format
 * This will be useful during the migration process
 */
export function migrateLegacyConfig(legacyConfig: unknown): Partial<EnhancedYAMLConfig> {
  const config = legacyConfig as Record<string, unknown>
  const enhanced: Partial<EnhancedYAMLConfig> = {
    metadata: {
  ...
  if (config.layers) {
    ...
    for (const [key, layer] of Object.entries(layers)) {
      const layerObj = layer as Record<string, unknown>
      const metadata = layerObj.metadata as Record<string, unknown> | undefined
      const keras = layerObj.keras as Record<string, unknown> | undefined
      
      enhanced.layers![key] = {
        metadata: {
          ...metadata,
          info: '/docs/general-info.md' // Default layer info - only for layers
        } as LayerMetadata,
        visual: layerObj.visual as VisualConfig,
        parameters: layerObj.parameters as Record<string, ParameterDefinition>,
        frameworks: {
          // Convert legacy keras section to new frameworks structure
          keras: {
            import: keras?.import as string || '',
            template: keras?.code_template as string || '',
            shape_computation: keras?.shape_computation as string || ''
          }
        }
      } as EnhancedLayer
    }
 ...
 * Useful for creating new configurations from scratch
 */
export const DEFAULT_ENHANCED_CONFIG: Partial<EnhancedYAMLConfig> = {
  metadata: {
    version: '2.0.0',
    description: 'Enhanced BlockDL Layer Configuration',
    framework: 'multi',
