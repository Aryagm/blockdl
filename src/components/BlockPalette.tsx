...
import { getAllTemplates, templateCategories, getTemplateCategoryColors, type NetworkTemplate } from "../lib/templates"
...
export default function BlockPalette({ className = "" }: BlockPaletteProps = {}) {
  ...
  const updateData = useCallback(() => {
    ...
    console.log(`🔄 BlockPalette updateData: ${types.length} types, ${categories.length} categories, ${allTemplates.length} templates`)
  ...
  const handleTemplateDragStart = useCallback((event: React.DragEvent, templateId: string) => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (currentTypes.length > 0 && currentCategories.length > 0 && currentTemplates.length > 0 &&
  ...
  const hasNoResults = (activeTab === CONFIG.TABS.LAYERS ? filteredCategories.length === 0 : Object.keys(templatesByCategory).length === 0) && searchTerm
  ...
  return (
    <div className={`space-y-6 p-6 h-full overflow-y-auto bg-slate-50/80 ${className}`}>
      ...
      <div className="relative">
        ...
        <Input
          type="text"
          placeholder={activeTab === CONFIG.TABS.LAYERS ? "Search layers..." : "Search templates..."}
          value={searchTerm}
      ...
      {hasNoResults ? (
        <div className="text-center py-8 text-slate-500">
          ...
          <p>No {activeTab === CONFIG.TABS.LAYERS ? 'layers' : 'templates'} found matching "{searchTerm}"</p>
        </div>
      ) : (activeTab === CONFIG.TABS.LAYERS ? filteredCategories.length === 0 : Object.keys(templatesByCategory).length === 0) ? (
        <div className="text-center py-8 text-slate-500">
          <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-slate-400 mx-auto mb-2"></div>
          <p>Loading {activeTab === CONFIG.TABS.LAYERS ? 'layers' : 'templates'}...</p>
        </div>
      ) : (
          ...
          {activeTab === CONFIG.TABS.LAYERS ? (
            ...
            Object.values(templatesByCategory).map((categoryData) => (
              <div key={categoryData.category} className="space-y-3">
                <h3 className={`text-sm font-medium ${categoryData.textColor} border-b border-slate-200 pb-1 flex items-center gap-2`}>
